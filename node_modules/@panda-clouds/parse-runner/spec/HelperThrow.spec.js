...
describe('helper classes throwing errors ', () => {
	...
	it('should propigate to test', async () => {
		expect.assertions(1);
		// const key = await parseRunner.callHelper('throwBadNumbers');

		// expect(key).toBe('We should never get here');
		await expect(parseRunner.callHelper('throwBadNumbers')).rejects.toThrow('Bad Numbers!');
	});
});
