...
describe('test Mongo inserts', () => {
	...
	it('should throw insertOne', async () => {
		...
		try {
			...
			expect(e.message).toBe('object must be of type object');
		}
	});
	...
	it('should throw insertMany', async () => {
		...
		try {
			...
			expect(e.message).toBe('objects parameter must be an array');
		}
	});

	it('should throw insertMany when bad object', async () => {
		expect.assertions(1);
